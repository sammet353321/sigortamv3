...
begin
    ...
    with company_stats as (
        select 
            coalesce(nullif(sirket, ''), 'Diğer') as sirket_fixed,
            sum(net_prim) as total_vol, 
            count(*) as c
    ...
    select jsonb_agg(jsonb_build_object('name', sirket_fixed, 'value', total_vol, 'count', c))
    into v_company_distribution
    from company_stats;

    -- Product Distribution (By Tur/Brans) - Fix for empty/null values
    with product_stats as (
        select 
            coalesce(nullif(tur, ''), 'Belirsiz') as tur_fixed,
            sum(net_prim) as total_vol, 
            count(*) as c
        from public.policeler
        where tanzim_tarihi >= v_year_start and tanzim_tarihi < v_year_end
    ...
    select jsonb_agg(jsonb_build_object('name', tur_fixed, 'value', total_vol, 'count', c))
    ...
    -- Monthly Trend (Includes Commission)
    with months as (
    ...
    select jsonb_agg(
        jsonb_build_object(
            ...
            'commission', coalesce(md.total_commission, 0),
