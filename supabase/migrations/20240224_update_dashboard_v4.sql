...
begin
    ...
    -- Company Distribution (ALL companies, with count and net_premium)
    ...
    -- Product Distribution (New: By Tur/Brans)
    ...
    -- Employee Performance (Use NET PRIM to match summary)
    with emp_stats as (
        select 
            u.name,
            count(p.id) as policy_count,
            coalesce(sum(p.net_prim), 0) as total_premium, -- Changed from brut_prim to net_prim
        ...
        -- Removed LIMIT to ensure total matches if possible, or keep limit if UI constrained
        -- User asked why totals don't match. Removing limit helps debugging, but UI might get long.
        -- Let's keep top 20, usually enough.
        limit 20
    )
    ...
    -- Monthly Trend (For selected year) - Using NET PRIM
    with months as (
    ...
    monthly_data as (
        select 
            ...
            sum(net_prim) as total, -- Changed to net_prim
            count(*) as count
        from public.policeler
        where tanzim_tarihi >= v_year_start and tanzim_tarihi < v_year_end
        group by 1
    ...
    select jsonb_agg(
        jsonb_build_object(
            ...
            'month_num', extract(month from m.m), -- Add numeric month for sorting/localization in UI
    ...
    values (
        ...
        jsonb_build_object(
            ...
            'product_distribution', coalesce(v_product_distribution, '[]'::jsonb), -- New
